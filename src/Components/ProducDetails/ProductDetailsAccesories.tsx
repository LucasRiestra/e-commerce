...
const ProductDetailAccesories = () => {
  const { productId } = useParams(); 
  const { addToCart } = useCart();
  const [similarProducts, setSimilarProducts] = useState<Product[]>([]);

  const productIdAsNumber = productId ? parseInt(productId, 10) : undefined;
  const product = AccessoriesArray.find((product) => product.id === productIdAsNumber);
  ...
  useEffect(() => {
    ...
    const randomIndices = getRandomIndices(AccessoriesArray.length, 3);
    const similarProducts = randomIndices.map(index => AccessoriesArray[index]);
      ...
      <h2>Similar Products</h2>
          ...
          {similarProducts.map((similarProduct, index) => (
            <div className="similar-product" key={index}>
              <Link to={`/detailAccesories/${similarProduct.id}`}>
...
export default ProductDetailAccesories;
