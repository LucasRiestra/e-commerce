...
const ProductDetailMen = () => {
  ...
  const product = MenProductsArray.find((product) => product.id === productIdAsNumber);
  ...
  useEffect(() => {
    ...
    const randomIndices = getRandomIndices(MenProductsArray.length, 3);
    const similarProducts = randomIndices.map(index => MenProductsArray[index]);
      ...
      <h2>Similar Products</h2>
          ...
          {similarProducts.map((similarProduct, index) => (
            <div className="similar-product" key={index}>
              <Link to={`/detailMen/${similarProduct.id}`}>
